...
- **`java -jar` means the manifest is the classpath** — the server launches via the manifest's version-pinned Class-Path. Upstream dep bumps (e.g. sqlite-jdbc 3.32→3.53) must ship with the mage jars or boot dies ("No suitable driver"). deploy-fork.sh now stages manifest deps and reconciles every lib dir.
- **Upstream renames plugin artifactIds** (2026-08: `booster-draft`→`boosterdraft`, `momir`→`momirfreeforall`, finalName→artifactId cleanup). The live/bundle `config.xml` are fork-frozen copies — jar renames strand them. deploy-fork.sh now verifies every config-referenced plugin exists (auto-copies from stage, fails loudly on true renames so config gets sed-fixed).
- **Fork-only Maven modules** (Mage.Bridge) aren't touched by upstream's version-bump — full-deploy.sh realigns their parent pom after each sync. Symptom if missed: "Non-resolvable parent POM" on a fresh `~/.m2` only.
- **Deploys must run detached on the server** (`nohup … > log`) — an ssh-tethered build dies with the connection. And never `pkill -f` a pattern contained in your own ssh command line.
- **`/home/user/dist/bundle` (the download-bundle staging dir) must exist** — it's not tracked in git and looks like disposable build output, but `deploy-fork.sh` re-zips the launcher update from it every deploy. It was deleted in the 2026-08-04 home-directory cleanup (mistaken for stale duplicate output) and broke `full-deploy.sh` on 2026-08-05 (bare `cd` under `set -e` killed the script *after* jars were already swapped into the live server but *before* `systemctl restart` — server never disrupted, but bundle/launcher-zip/config.json update never happened). Recovery: `mkdir -p ~/dist/bundle && cd ~/dist/bundle && unzip /var/www/html/files/mage-update_fork.zip` to reseed it from the last known-good bundle, then re-run the deploy. The re-zip step now fails loudly with this exact recovery command instead of a bare stack trace.
